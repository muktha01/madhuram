...
function Couple() {
  ...
  return <section className="couple section-frame" ref={ref}><Frame /><img className="scenery scenery-top" src={`${ASSET}/temple-scenery.png`} alt="" loading="lazy" decoding="async" /><img className="scenery scenery-bottom" src={`${ASSET}/temple-scenery.png`} alt="" loading="lazy" decoding="async" /><div className="section-inner"><h2 className="reveal">The Couple</h2><p className="section-sub reveal">Two families, many blessings, one timeless celebration.</p><div className="reveal"><CoupleCard name="Vijay Deverakonda" role="Son of" parents="Deverakonda Govardhan Rao & Deverakonda Madhavi" image={`${PHOTO_BASE}/618dff4e0d-534536433_18324616675238653_1023307852965625301_n.jpg`} /></div><img className="divider reveal" src={`${ASSET}/gold-divider.webp`} alt="" loading="lazy" decoding="async" /><div className="reveal"><CoupleCard name="Rashmika Mandanna" role="Daughter of" parents="Madan Mandanna & Suman Mandanna" image={`${PHOTO_BASE}/c4cc91ddfb-654209778_18573193186045304_3648194887087051682_n.jpg`} mirror /></div></div></section>;
}
...
function CoupleCard({ name, role, parents, image, mirror = false }: { name: string; role: string; parents: string; image: string; mirror?: boolean }) {
  return <div className="couple-unit"><div className={`couple-card ${mirror ? 'mirror' : ''}`}><div className="halo" /><div className="photo-window"><img src={image} alt={name} loading="lazy" decoding="async" /></div><img className="ornament" src={`${ASSET}/peacock-arch.webp`} alt="" loading="lazy" decoding="async" /></div><div className="couple-meta"><h3>{name}</h3><p className="role">{role}</p><p className="parents">{parents}</p></div></div>;
}
...
function Countdown() {
  ...
  return <section className="countdown"><div className="countdown-art"><img src={`${ASSET}/countdown-wall.webp`} alt="" loading="lazy" decoding="async" /><div className="countdown-values">{units.map((value, index) => <div key={index}><strong>{String(value).padStart(2, '0')}</strong><span>{['Days', 'Hours', 'Minutes', 'Seconds'][index]}</span></div>)}</div></div></section>;
}

function Events() {
  const ref = useReveal<HTMLDivElement>();
  return <section className="events section-frame" ref={ref}><Frame /><div className="kolam" /><div className="section-inner"><h2 className="reveal">The Wedding Journey</h2><p className="section-sub reveal">Walk with us, function to function, to the sacred hour.</p><div className="event-list">{events.map((event, index) => <article className={`event reveal ${index % 2 ? 'right' : 'left'}`} key={event.name} style={{ transitionDelay: `${index * 120}ms` }}><div className="event-icon"><img src={event.image} alt="" loading="lazy" decoding="async" /></div><div className="event-card"><h3>{event.name}</h3><p className="event-when">{event.date} <span>·</span> {event.time}</p><p className="event-venue">ITC Mementos, Udaipur</p><a href="https://maps.app.goo.gl/XaQmk5ifYLZmsPuAA" target="_blank" rel="noreferrer"><MapPin size={11} /> View Map</a></div></article>)}</div></div></section>;
}

function Gallery({ onOpen }: { onOpen: (index: number) => void }) {
  const ref = useReveal<HTMLDivElement>();
  return <section className="gallery" ref={ref}><div className="gallery-toranam" /><div className="gallery-motes" aria-hidden="true">{Array.from({ length: 12 }).map((_, i) => <span key={i} className="gallery-mote" />)}</div><img className="gallery-parasol" src={`${ASSET}/gallery-parasol.png`} alt="" aria-hidden="true" loading="lazy" decoding="async" /><div className="section-inner"><h2 className="reveal">Before the Vows</h2><p className="section-sub reveal">Little moments from the years that brought us here.</p><div className="gallery-grid">{galleryImages.map((image, index) => <button className={`gallery-cell reveal ${index === 0 || index === 9 ? 'feature' : ''}`} key={image} style={{ transitionDelay: `${(index % 2) * 100}ms` }} onClick={() => onOpen(index)}><img src={image} alt="" loading="lazy" decoding="async" /><span /></button>)}</div></div></section>;
}
...
function VideoSection() {
  ...
  return <section className="video-section"><div className="video-wall"><img src={`${ASSET}/${currentTheme.videoWall}`} alt="" loading="lazy" decoding="async" /><button className="video-opening" onClick={() => setPlaying(true)} aria-label="Play wedding video"><Play size={30} fill="currentColor" /></button></div>{playing && <div className="video-modal" onClick={() => setPlaying(false)}><button onClick={() => setPlaying(false)} aria-label="Close video"><X /></button><div className="video-modal-card"><img src={galleryImages[0]} alt="Wedding memories" /><div><Play size={28} fill="currentColor" /><p>A Special Glimpse</p></div></div></div>}</section>;
}

function Footer() { return <footer><img src={`${ASSET}/footer-card.jpg`} alt="" loading="lazy" decoding="async" /><div className="footer-greeting"><p>We await your gracious presence</p><h2>and your blessings</h2><span>శుభమస్తు</span></div><a href="https://myshaadhilink.in" target="_blank" rel="noreferrer">This invitation was crafted on <b>MyShaadhi Link</b><small>Need one for your wedding? Click here</small></a></footer>; }
